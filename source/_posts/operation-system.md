...
- 死锁的处理策略
  ...
  - 避免死锁
    ...
    - 银行家算法：在进程申请资源时，预判这次分配后会不会进入**不安全状态**，如果会则阻塞进程

      > 不安全状态：找不到安全序列
      >
      > > 安全序列：系统按照这个序列分配资源，则每个进程都能顺利完成

  - 死锁的检测：根据资源分配图是否可完全简化判断（安全序列）
...
-   <u>编译、链接、装入</u>
    ...
    -   概念
        ...
        -   链接：<u>链接程序</u>将目标模块与**库函数**链接在一起，形成的**装入模块**
    ...
    -   链接的三种方式
        ...
        -   静态链接：在**程序运行前**将目标模块与库函数链接成**完整的装入模块**

        -   装入时动态链接：在目标模块**装入内存时**链接库函数
...
- <u>页表与地址结构</u>
  ...
  - 页表：操作系统为每个进程建立一张页表，记录**进程每个页面实际在内存中存放的内存块号**
  ...
  - 页表项：每个页表项**长度相等**，由页号和块号组成
    ...
    - 块号：实际存放到的**内存块号**，也是进**程页起始地址**。其位数由**内存块的数量**决定
  ...
  - **快表**：又称联想寄存器（TLB），是一种**高速缓存**，用于存放最**近访问过的若干页表项**，以加速地址变换。相对而言，内存中的页表常称为**慢表**
...
- <u>地址变换机构</u>
  ...
  - 地址变换过程
    ...
    2. 根据页号得到**进程页起始地址**
       ...
       1. **越界检查**：将页号与页表长度对比，检查页号是否合法

       2. **索引计算**：根据页表起始地址、页表项长度、页号计算出**页表项地址**

          > 这也是**数组**索引访问的计算原理

       3. **得到始址**：由页表项得到**进程页起始地址**

       4. （**更新快表**：将页表项存入快表）

    3. 根据**进程页起始地址和页内偏移量**计算出**物理地址**

    > 页式管理中地址是**一维**的，只要给出**一个逻辑地址**就可以自动计算出**物理地址**
