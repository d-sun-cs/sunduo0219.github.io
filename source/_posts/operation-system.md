...
- 硬件实现
  ...
  - 中断屏蔽：在访问临界区前后**关开中断**
  - TestAndSet指令（TS、TSL）：访问临界区前**“上锁”并检查之前是否已经有锁**（这个操作由硬件完成，不可中断），若有则循环等待，若无则访问；**访问临界区后”解锁“**
  - Swap指令（Exchange、XCHG）：访问临界区前查之前是否已经有锁并”上锁“，若有则循环等待，若无则访问；**访问临界区后”解锁“**
...
- 经典问题及其解决思路
  ...
  - 读者-写者问题
    - 写-写互斥：rw锁
    - 写-读互斥：rw锁
    - 读-读不互斥：根据count变量加解rw锁，并通过mutex锁保证对count的更改和判断是不可中断的
    - 读-写公平：再加一个w锁约束读进程对rw锁的加锁
