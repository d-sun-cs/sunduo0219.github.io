...
-   FOLLOW集
    ...
    -   计算：不断包含**右边的FISRT集**和作为右部结尾时**左部的FOLLOW集**
...
-   基本思想与概念
    ...
    -   **等价**LR(1)项目/**展望符**的计算：
        ...
        -   第二分量/展望符：要看等价项目产生式的**左部**在原项目中**右边的符号(串)**$$\beta$$。若$$\beta$$能推出空（或者$$\beta$$本身就是空的），则直接继承相同的展望符构成项目；同时，对于$$FIRST(\beta)$$中的每个终结符，都可以作为**展望符**与相同的**第一分量**构成一个等价的项目

        -   >   从计算展望符的角度讲，就要从初始项目$$S'\rarr\cdot S,\$$$出发按照自动机的构造方法去计算了；
            >
            >   第一分量相同的项目，可以合并到一起写，**展望符用/隔开**，但要知道这是多个项目
...
-   访问链的建立：
    ...
    -   建立步骤：建立访问链，其实就是在**调用序列**中加入相关代码，由**调用者**负责建立访问链
        ...
        -   如果$$n_x，即**外层调用内层**，那么需要在y的访问链中放置一个指向x的活动记录的指针
