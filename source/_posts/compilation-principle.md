...
- 非终结符FIRST集的计算
  ...
  >
  > - 如果要求**文法符号串的FIRST集**，从左向右看，加入第一个符号的FIRST集，若该符号能推导出$\epsilon$就累加继续下一个符号的FIRST集，直到不能再推出$\epsilon$，如果都能推导出$\epsilon$就再加入$\epsilon$
...
- 非递归的预测分析法：借助**下推自动机**，初始时，栈顶为开始符，栈底为结束符
  ...
  > 非递归的，也叫**表驱动的预测分析法**，<u>预测分析表</u>的定义如下：
  >
  > - 行为非终结符，列为输入符号
