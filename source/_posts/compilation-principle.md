...
- **<u>自顶向下</u>构造语法分析树**，每次用**最左非终结符**（树的最左非终结符节点），根据**输入符**，选择其合适的**产生式**，进行替换（生成子节点）
...
- FIRST集的计算
  ...
  1. 处理所有**空产生式**和**右部以终结符开头**的产生式
  2. 处理**右部以非终结符开头**的产生式
     - 始终保持**右部开头**非终结符的FIRST集包含于该产生式**左部**非终结符的FIRST集
     - 若**右部开头**非终结符的FIRST集中有$\epsilon$，则要继续保持**紧邻右边符号**的FIRST集包含于该产生式**左部**非终结符的FIRST集，以此递归，直到不出现$\epsilon$

  > 上面说的是文法符号的FIRST集，文法符号串的FIRST集很简单，从左向右看，出现$\epsilon$就累加即可，如果都能推导出$\epsilon$就再加入$\epsilon$

- FOLLOW集

  1. 将**结束标记**加入开始符号的FOLLOW集中
  2. 不断遍历所有产生式，直到无法再添加新的终结符到某一FOLLOW集中，添加规则为：
     - 单看**产生式右部**，跟在某一非终结符**后面的符号串的FIRST集**加入该终结符的FOLLOW集中（除了$\epsilon$）
     - 看**整条产生式**，**右部结尾**的非终结符，以及可能因右方非终结符推导出$\epsilon$而**变成右部结尾**的非终结符，其FOLLOW集要**包含**左部非终结符的FOLLOW集
...
- 递归的预测分析法
  ...
  - 从文法开始符号的产生式开始，遍历右部符号（即**调用文法开始符号的过程**）
...
- 非递归的预测分析法（借助**下推自动机**，初始时，栈顶为开始符，栈底为结束符）
  ...
  - 若栈顶符号为**终结符**
    - 二者**相等**，则弹栈，读头后移
    - 二者不相等，则报错
  - 若栈顶符号为**非终结符**：根据SELECT集选择**产生式**，根据产生式弹栈并入栈
...
- 自底向上分析也叫**移入-归约分析**
  ...
  - 移入：将**输入符号**压入栈
  - 归约：从栈中依次**弹出**能构成**产生式右部**的若干个符号，然后将**左部压入栈**
...
- **LR分析**是移入-归约分析的重要方法
  ...
  - 只要构造出正确的**<u>分析表</u>**就可以直观且快速地识别了，分析表的概述如下：
    ...
    - 表项中，$s_n$代表将**所在列对应符号和状态n**同时压入栈符号栈和状态栈；$r_n$代表使用第n个产生式规约，同时弹出**产生式右部**符号(串)和与其**平行的状态**，并压入左部非终结符，再根据**当前栈顶状态**和**该非终结符**找到GOTO列组的项k，这代表着再**将状态k压入栈**；acc代表成功识别，err代表出错
...
- LR(0)相关<u>概念</u>
  ...
  - **项目**：项目描述了**句柄识别的状态**
    ...
    - 移进项目
      ...
      > 开始符号对应的移进项目称为<u>初始项目</u>

    - 待约项目

    - 归约项目

      > 开始符号对应的归约项目称为<u>接收项目</u>

  - **后继项目**：同属一个产生式但圆点只差一个符号，圆点在后的是后继项目

  - **等价的项目/项目集闭包**：紧邻圆点右方的文法符号<u>相同</u>或<u>分别出现在某条**产生式**的**左部**和**右部串首**</u>的项目为等价的项目。等价的项目有传递性，这样划分出的等价类也叫**项目集闭包**。

  - 项目集对应于**文法符号X(包括终结符和非终结符)**的**后继项目集闭包**（`GOTO(I, X)`函数）：项目集中**圆点右紧邻符号为X**的每一个项目的**后继项目的项目集闭包的并**

- LR(0)<u>自动机与分析表</u>

  - 自动机的构造

    1. 初始项目的**项目集闭包**构成**状态0**
...
- LR(0)会有**移进/归约**冲突和**归约/归约**冲突
  ...
  - 对应到自动机中，就是一个状态中**同时存在规约项目和其他类型项目**，或者同时存在**多个规约项目**，导致遇到下一个输入时无法判断执行什么动作
...
- SLR分析基本思想
  ...
  - 是对LR(0)分析的改进，**在LR(0)分析的基础上**，不是遇到所有输入符号都能归约的，因为归约后左部符号的**FOLLOW集**中可能没有输入符，也就是归约后的符号后面不可能紧接这个输入符
...
- SLR<u>自动机与分析表</u>
  ...
  - 分析表的构造：在LR(0)构造的分析表基础上，只有**ACTION列的终结符**在对应状态中归约项目的**FOLLOW集**中时，才进行归约，否则的话，要么是**移进**，要么**表项为空**
